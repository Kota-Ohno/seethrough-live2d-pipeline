...
解説記事: [絵が描けなくても2日でできた — AIイラスト1枚からComfyUI-See-throughで配信用Live2Dモデルを作る全記録 (Zenn)](https://zenn.dev/shakebenn/articles/e9b4c797d103ed)

## パイプライン

繰り返し制作する際は、[リギング前の標準作業フロー](docs/pre-rig-production-workflow.md)を先に参照してください。実際の試行で発生した領域混入、輪郭のぼけ・へこみ、瞳と影、隠れ面、PSD検証の失敗と対処を整理しています。[失敗切り分け表](docs/pre-rig-failure-routing.md)と[チェックポイントのテンプレート](docs/templates/pre-rig-checkpoint.md)を併用すると、同じ確認や修正を繰り返す負担を減らせます。

これらは工程と検証の指針です。このリポジトリに全工程の自動実装が揃っている、または静止PSDだけでリギング品質を保証できる、という意味ではありません。

後段の制作記録として、[リギング・実動作・紹介撮影で得た知見](docs/rigging-runtime-recording-lessons.md)も参照してください。足首やこめかみの素材混入、三つ編みの重力追従、強調した揺れ、実動作の検証、録画確認と非同期カンペ運用を整理しています。

判断の根拠は[構造化した制作ログ](evidence/README.md)に保存しています。匿名化した元レポート、対象ファイル一覧、ハッシュ、失敗・未完了を含む状態を追跡でき、次回用の収集・整合性検証スクリプトも付属します。
